...
describe('Blog detail page', () => {
  it('Should render detailed content of ID1', async () => {
    const { page } = await getPage({ route: '/posts/1' })
    render(page)

    expect(await screen.findByText('dummy title1 1')).toBeInTheDocument()
    expect(screen.getByText('dummy body 1')).toBeInTheDocument()
  })
  it('Should render detailed content of ID2', async () => {
    const { page } = await getPage({ route: '/posts/2' })
    render(page)

    expect(await screen.findByText('dummy title1 2')).toBeInTheDocument()
